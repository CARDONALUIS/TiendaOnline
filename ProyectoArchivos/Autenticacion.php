...
{
	extract($_GET);//Se crea $txtUsuario y paswword
	if($txtUsuario != "" && $txtPassword != "")
	{
		//Se hace la verificacion de los datos en la BD
		$conn = mysqli_connect("localhost", "root", "", "proyecto2");
		
		echo $qry = "select idUsuario,Nombre,isAdmi from usuarios where Usuario='$txtUsuario' and Pwd='$txtPassword'";
		
		$rs = mysqli_query($conn, $qry);		
		if(mysqli_num_rows($rs)>0 )//Si encontro conicidencia
		{
			$usr = mysqli_fetch_array($rs);
			$_SESSION['idUsuario'] = $usr["idUsuario"];
			$_SESSION['Nombre'] = $usr["Nombre"];
			$_SESSION['isAdmi'] = $usr["isAdmi"];
			header("Location:http://localhost/Proyecto/portada.php");

		}
		else// el usuario y el password no exiten
		{
...
function VerificarFRMAute()
{
	...
	{
		alert("El password no se puede omitir");
		return false;
	}
	return true;
...
<div class="layoutPlan" >
 ...
 <div class="Autenticate">
	...
	<h1>Autenticacion</h1>
	<form action="Autenticacion.php" method="get" onsubmit="return VerificarFRMAute()">
		<div class="camposRegis">
			<input type="text" name="txtUsuario" id="txtUsuario" placeholder="Usuario"><br>
			<input type="password" name="txtPassword" id="txtPassword" placeholder="Contraseña"><br>
		</div>
	
		<div class="botonReg">
			<input type="submit" value="Autenticarme">
		</div>
		<br>
