...
<?php
	...
	if(isset($_SESSION['idUsuario']))//Si no te has autenticado te lleva a la portada
	{
		...
		if(mysqli_num_rows($resultado )>0 )//Si encontro conicidencia
		{
			...
			//echo "<a href='agregaProd.php?idC=".$_GET['idC']."'">Agrega Producto"</a>";
			//if(isset($_SESSION['idUsuario']))
			echo "<a href='AgregaProd.php?idC=".$_GET['idC']."'>Agrega Producto</a> <br>";
			echo "<a href='ElimiProd.php?idC=".$_GET['idC']."'>EliminaProd</a>";
	...
	//$consulta = "SELECT *FROM venta INNER JOIN producto ON venta.idProducto=producto.idProducto AND venta.idUsuario=".$_SESSION['idUsuario']."";
			...
			//$consulta = "select * from clasificacion INNER JOIN producto on clasificacion.idClasificacion = producto=idClasificacion";					
?>
	...
	if(isset($_GET['idC']) && $_GET['idC'] != "")
	{
		...
		$consulta = "SELECT * FROM clasificacion where idClasificacion=".$_GET['idC']."";
		
		$rs = mysqli_query($conPro , $consulta );
		$registro = mysqli_fetch_array($rs);
		
		echo "<h1>".$registro['clasificacion']."</h1>";
				 	
	 	$consulta2 = "SELECT *FROM clasificacion INNER JOIN producto ON clasificacion.idClasificacion=producto.idCategoria AND clasificacion.idClasificacion=".$_GET['idC']."";
	...
	<!--
	<div>
		<img src="Imagenes/Play4.jpg" alt="play4">
		<p class="nomPro">Producto</p>
		<p class="precioPro">$100</p>
		<a href="#">COMPRAR</a>	
	</div>
	
	<div>
		<img src="Imagenes/xbox.jpg" alt="play4">
	</div>
	
	<div>
		<img src="Imagenes/switchP1.jpg" alt="play4">
	</div>
	
	<div>
		<img src="Imagenes/play4P1.jpg" alt="play4">
	</div>
